...
export default function AdminTravelTips() {
  ...
  const handleFileChange = (file: File | null) => {
    if (!file) {
      setFormData((prev: any) => ({ ...prev, file: null, filePreview: null }));
      return;
    }
  ...
  const handleSave = async () => {
    ...
    if (!formData.title?.trim()) return alert("Please enter title");
    ...
    if (formData.category === "card") {
      ...
      if (!formData.id && !hasNewFile)
        return alert("Please select an image for Card tips");
      if (formData.id && !hasNewFile && !hasPreview)
        return alert("Card tips must have an image");
    }
    ...
    <div className="min-h-screen flex flex-col bg-[#1e1313] text-white">
      ...
      <main className="flex-1 pt-28 px-6 lg:px-20 space-y-10 container mx-auto">
        <h1 className="text-3xl font-bold text-pink-400">
          ✈️ Manage Travel Tips
        </h1>

        {/* Form */}
        <Card className="bg-[#2a1a1a] border border-pink-600">
          <CardContent className="p-6 space-y-4">
            ...
            {formData.category === "card" && (
              <>
                <Input
                  placeholder="Preview Text (shown under image)"
                ...
                <div>
                  ...
                  {formData.filePreview && (
                    <img
                      ...
                      alt="Preview"
                      className="mt-3 w-40 h-28 object-cover rounded-md border border-gray-600"
                    />
                  )}
                </div>
              </>
            ...
            <Button className="mt-4 bg-pink-600 hover:bg-pink-700" onClick={handleSave} disabled={saving}>
        ...
        {loading ? (
          ...
          <div className="grid gap-6">
            {tips.map((tip) => (
              <Card
                key={tip.id}
                className="bg-[#2a1a1a] border border-pink-600 shadow-md hover:shadow-lg transition"
              >
                <CardContent className="p-6 space-y-2">
                  <div className="flex justify-between items-center">
                    <h2 className="text-lg font-semibold">{tip.title}</h2>
                    <span className="text-sm text-gray-400">
                      {tip.category} / {tip.icon}
                    </span>
                  </div>
